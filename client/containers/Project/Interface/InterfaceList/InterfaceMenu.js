...
class InterfaceMenu extends Component {
  ...
  enterAPIItem = id => {
    this.setState({ delAPIIcon: id });
    this.leaveItem()
  };

  leaveAPIItem = () => {
    this.setState({ delAPIIcon: null });
  };
  ...
  onDrop = async e => {
    ...
    if (dragKey.indexOf('cat') !== -1) {
      message.error("分类不支持拖拽");
      return;
    }
    if (dropKey === 'root') {
      message.error("不能放到该分类");
      return;
    }
    // 这里的 list 实际上是一个两层的数组： 分类 和 接口
    ...
    if (dropKey.indexOf('cat') === -1) {
      ...
      // 目标是某个分类目录
      targetType = 'folder'
      dropCatId =  dropKey.replace('cat_', '') * 1
    }
    const id = e.dragNode.props.eventKey;
    const dropPos = e.node.props.pos.split('-');
    let dropIndex = Number(dropPos[dropPos.length - 1]);
    const dragPos = e.dragNode.props.pos.split('-');
    let dragIndex = Number(dragPos[dragPos.length - 1]);

    if (dragKey.indexOf('cat') === -1) {
      console.log(123456);
      if (dropCatId === dragCatId) {
        console.log(123456);
        // 同一个分类下的接口交换顺序(前提是目标不是自己所在的目录而是自己的兄弟)
        if (targetType === 'folder') {
          return message.error("已在该分类下");
        }
        console.log(123456);
        let colList = [];
        list.forEach(x => {
        ...
        // 由于混入了api 和 分类，因此索引就不对，需要手动纠正
        if (dragIndex >= colList.length) {
          dragIndex = dragIndex - colList.length
        }
        if (dropIndex >= colList.length) {
          dropIndex = dropIndex - colList.length
        }
      ...
      return (
        <TreeNode
          title={
            <div
              className="container-title"
              onMouseEnter={() => this.enterAPIItem(item._id)}
              onMouseLeave={this.leaveAPIItem}
            >
              <Link
              ...
              <div className="btns">
                <Tooltip title="删除接口">
                  <Icon
                    ...
                    style={{ display: this.state.delAPIIcon == item._id ? 'block' : 'none' }}
                  />
                </Tooltip>
                <Tooltip title="复制接口">
                  <Icon
